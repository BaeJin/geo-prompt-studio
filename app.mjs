import {generate,toCSV,keyOf,valuesOf,migrateStudio,validateStudio,loadTemplate,loadParameters,combinationConfig} from './engine.mjs';
import {sample} from './sample.mjs';
const $=id=>document.getElementById(id), uid=()=>crypto.randomUUID();
const esc=text=>String(text??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const STORAGE='datacast-prompt-studio-v2', LEGACY='datacast-prompt-studio-v1';
let state=migrateStudio(sample()), rows=[],selected=new Set(),page=0,dirty=true,draft=null,draftKind=null,timer,confirmAction;
state.combinations=[];
try{const saved=localStorage.getItem(STORAGE)||localStorage.getItem(LEGACY);if(saved)state=migrateStudio(JSON.parse(saved));}
...
function renderLibrary(){
  const {templateSets,parameterSets}=state.library;
  $('template-badge').textContent=templateSets.length;$('parameter-badge').textContent=parameterSets.length;
  ...
  $('parameter-library').innerHTML=parameterSets.map(p=>`<article class="library-card"><div class="card-top"><span class="ingredient-icon parameter-icon" aria-hidden="true">{ }</span><h3>${esc(p.name)}</h3><span class="set-meta">${p.params.length}개 변수</span></div><div class="parameter-snippets">${p.params.map(v=>`<div><code>${esc(v.key)}</code><span>${esc(valuesOf(v.values).slice(0,3).join(' · '))}${valuesOf(v.values).length>3?' 외 '+(valuesOf(v.values).length-3)+'개':''}</span></div>`).join('')}</div><div class="card-bottom"><span class="hint">저장된 값</span><div><button class="quiet small" data-library="parameters" data-id="${esc(p.id)}" data-action="edit">수정</button><button class="quiet small" data-library="parameters" data-id="${esc(p.id)}" data-action="delete">삭제</button></div></div></article>`).join('')||'<p class="empty-hint">국가, 차종 등 파라미터를 만들어 보세요.<br>템플릿에 연결하지 않고 저장합니다.</p>';
...
function libraryArray(kind){return state.library[kind==='templates'?'templateSets':'parameterSets'];}
function openEditor(kind,id){
  draftKind=kind;const existing=libraryArray(kind).find(v=>v.id===id);
  draft=existing?structuredClone(existing):kind==='templates'?{id:uid(),name:'',templates:[{id:uid(),name:'질문 1',text:''}]}:{id:uid(),name:'',params:[{key:'',values:''}]};
  $('editor-title').textContent=(kind==='templates'?'템플릿':'파라미터')+(existing?' 수정':' 만들기');$('draft-name').value=draft.name;
  $('draft-help').textContent=kind==='templates'?'값이 들어갈 자리는 {Country}, {Model}처럼 작성하세요. 파라미터 연결은 조합 화면에서 합니다.':'이름에는 중괄호를 빼고, 값은 한 줄에 하나씩 넣으세요. 사용할 템플릿은 나중에 선택합니다.';
  $('add-draft-item').textContent=kind==='templates'?'＋ 질문 문장 추가':'＋ 파라미터 추가';$('draft-error').hidden=true;renderDraft();$('library-editor').showModal();$('draft-name').focus();
}
function renderDraft(){
  $('draft-items').innerHTML=draftKind==='templates'?draft.templates.map((t,i)=>`<div class="draft-block"><div class="draft-title"><label class="field-label" for="draft-label-${i}">질문 ${i+1} 이름</label><button type="button" class="quiet small" data-remove="${i}">삭제</button></div><input id="draft-label-${i}" class="text-input" data-field="name" data-index="${i}" value="${esc(t.name)}" maxlength="200"><label class="field-label" for="draft-text-${i}">질문 문장</label><textarea id="draft-text-${i}" data-field="text" data-index="${i}" maxlength="16000" placeholder="What is the best {Category} in {Country}?">${esc(t.text)}</textarea></div>`).join(''):draft.params.map((p,i)=>`<div class="draft-block"><div class="draft-title"><span class="field-label">파라미터 ${i+1}</span><button type="button" class="quiet small" data-remove="${i}">삭제</button></div><div class="draft-param"><div><label class="field-label" for="draft-key-${i}">이름</label><input id="draft-key-${i}" class="text-input" data-field="key" data-index="${i}" value="${esc(p.key)}" placeholder="Country" maxlength="100"></div><div><label class="field-label" for="draft-values-${i}">값 · 한 줄에 하나</label><textarea id="draft-values-${i}" data-field="values" data-index="${i}" maxlength="50000" placeholder="Australia">${esc(p.values)}</textarea></div></div></div>`).join('');
}
$('draft-name').oninput=e=>draft.name=e.target.value;
$('draft-items').oninput=e=>{if(e.target.dataset.field)(draftKind==='templates'?draft.templates:draft.params)[e.target.dataset.index][e.target.dataset.field]=e.target.value;};
$('draft-items').onclick=e=>{const button=e.target.closest('[data-remove]');if(button){(draftKind==='templates'?draft.templates:draft.params).splice(Number(button.dataset.remove),1);renderDraft();}};
$('add-draft-item').onclick=()=>{const list=draftKind==='templates'?draft.templates:draft.params;if(list.length>=(draftKind==='templates'?100:30))return toast('추가 가능한 항목 수를 초과했습니다.');list.push(draftKind==='templates'?{id:uid(),name:'질문 '+(list.length+1),text:''}:{key:'',values:''});renderDraft();};
...
$('library-form').onsubmit=e=>{
  ...
  try{
    ...
    if(draftKind==='templates'){
      ...
      const keys=draft.params.map(p=>keyOf(p.key));
      if(!keys.length||keys.some(k=>!k||/[{}]/.test(k))||new Set(keys).size!==keys.length)throw Error('파라미터 이름은 비어 있거나 중복될 수 없습니다. 중괄호는 빼주세요.');
      if(draft.params.some(p=>!valuesOf(p.values).length))throw Error('각 파라미터에 값을 하나 이상 입력하세요.');
    }
    const list=libraryArray(draftKind),index=list.findIndex(v=>v.id===draft.id);
    if(index<0&&list.length>=50)throw Error('라이브러리 세트는 각각 최대 50개까지 가능합니다.');
...
function renderCombinations(){
  ...
  $('combination-list').innerHTML=state.combinations.map((c,i)=>`<article class="combination-card" data-combo="${esc(c.id)}"><div class="combo-heading"><label class="check"><input type="checkbox" data-kind="combo-enabled" ${c.enabled?'checked':''}><strong>${String(i+1).padStart(2,'0')} · ${esc(c.templateSet.name)}</strong></label><button class="quiet small" data-action="remove-combo">빼기 ×</button></div><div class="loaded-templates">${c.templateSet.templates.map((t,j)=>`<label class="loaded-question"><input type="checkbox" data-kind="template-enabled" data-index="${j}" ${t.enabled?'checked':''}><span><b>${esc(t.name)}</b><span>${esc(t.text)}</span></span></label>`).join('')}</div><div class="connection-label">↓ 연결할 파라미터</div>${c.parameterSets.map((p,j)=>`<div class="loaded-parameter"><div><strong>${esc(p.name)}</strong><button class="quiet small" data-action="remove-param" data-index="${j}" aria-label="${esc(p.name)} 조합에서 빼기">×</button></div><p>${p.params.map(v=>`<code>${esc(v.key)}</code> ${valuesOf(v.values).length}개`).join(' · ')}</p><label class="mode-label">조합 방식 <select data-kind="mode" data-index="${j}" aria-label="${esc(p.name)} 조합 방식"><option value="product" ${p.mode==='product'?'selected':''}>전체 조합</option><option value="zip" ${p.mode==='zip'?'selected':''}>행별 매칭</option></select></label><p class="hint">${p.mode==='zip'?'같은 줄끼리 연결 · 값 1개는 모든 행에 적용':'템플릿에 사용된 값의 모든 조합'}</p></div>`).join('')}<div class="load-row"><select class="parameter-picker" aria-label="${esc(c.templateSet.name)}에 불러올 파라미터"><option value="">저장한 파라미터 선택</option>${state.library.parameterSets.filter(p=>!c.parameterSets.some(v=>v.id===p.id)).map(p=>`<option value="${esc(p.id)}">${esc(p.name)}</option>`).join('')}</select><button class="outline small" data-action="load-param">불러오기</button></div></article>`).join('')||'<div class="empty-composer"><span aria-hidden="true">T ＋ { }</span><h3>첫 번째 조합을 만들어 보세요</h3><p>위에서 저장한 템플릿을 불러온 다음,<br>질문에 넣을 파라미터를 연결하세요.</p></div>';
}
$('load-template').onclick=()=>{try{loadTemplate(state,$('template-picker').value,uid());markDirty();renderCombinations();toast('템플릿을 불러왔습니다. 파라미터를 연결하세요.');}catch(e){toast(e.message);}};
$('combination-list').onclick=e=>{const el=e.target.closest('[data-action]');if(!el)return;const id=el.closest('[data-combo]').dataset.combo,c=state.combinations.find(c=>c.id===id),{action,index}=el.dataset;try{if(action==='remove-combo')state.combinations=state.combinations.filter(v=>v.id!==id);if(action==='remove-param')c.parameterSets.splice(Number(index),1);if(action==='load-param')loadParameters(state,id,el.closest('[data-combo]').querySelector('.parameter-picker').value);markDirty();renderCombinations();}catch(error){toast(error.message);}};
$('combination-list').onchange=e=>{const el=e.target,{kind,index}=el.dataset;if(!kind)return;const c=state.combinations.find(c=>c.id===el.closest('[data-combo]').dataset.combo);if(kind==='combo-enabled')c.enabled=el.checked;if(kind==='template-enabled')c.templateSet.templates[index].enabled=el.checked;if(kind==='mode')c.parameterSets[index].mode=el.value;markDirty();renderCombinations();};
