import {validateData,availableAnalyses,selectAnalysis,entitiesOf,groupPrompts,coverage,csv,escapeHTML as esc,highlighted} from './model.mjs';
...
function setData(value){
  ...
  $('version').innerHTML='<option value="latest">응답별 최신 저장 결과</option>'+data.versions.slice().sort((a,b)=>b.responses-a.responses).map(v=>`<option value="${esc(v.id)}">${esc(v.name)} · ${v.responses}응답</option>`).join('')+'<option value="reference">AI 참조 초안 (별도 데이터)</option>';
...
function render(){
  ...
  $('counts').innerHTML=`<span>프롬프트<strong>${groups.length}</strong></span><span>응답<strong>${visible.length}</strong></span><span>${version()==='reference'?'참조 초안':'분석'}<strong>${analysed}/${visible.length}</strong></span>`;
  $('scope-note').textContent=version()==='latest'?'응답마다 가장 최근에 저장된 분석을 표시합니다. 서로 다른 Judge 버전과 실험 결과가 포함됩니다.':version()==='reference'?'AI가 작성한 참조 초안입니다. Judge 실행 결과 및 인간 검수 완료 자료와 구분됩니다.':'선택한 Judge 버전의 최신 저장 결과를 표시합니다. 결과 없음은 미언급을 의미하지 않습니다.';
  $('prompt-count').textContent=groups.length+'개 프롬프트';
  $('prompt-list').innerHTML=groups.map(g=>`<button class="prompt-item ${g.id===promptId?'active':''}" data-prompt="${esc(g.id)}" ${g.id===promptId?'aria-current="true"':''}><code>${esc(g.id)}</code><span>${esc(g.text)}</span><small>응답 ${g.responses.length} · ${version()==='reference'?'초안':'분석'} ${coverage(g.responses,version())}</small></button>`).join('')||'<div class="blank">검색 결과 없음</div>';
...
function renderPrompt(){
  ...
  $('analysis-picker').innerHTML=version()==='reference'?'<option>최신 참조 초안</option>':candidates.length?candidates.map(a=>`<option value="${esc(a.analysis_id)}" ${a.analysis_id===analysisId?'selected':''}>${esc(date(a.analyzed_at))} · ${esc(a.judge_name)}</option>`).join(''):'<option>저장 결과 없음</option>';
  $('analysis-picker').disabled=version()==='reference'||!candidates.length;
  renderAnalysis();
}
...
function renderAnalysis(){
  ...
  $('analysis-meta').innerHTML=!a?'':a.is_reference?`<span class="tag draft">${esc(a.author_kind)} · ${esc(a.review_status)}</span> · revision ${a.revision}${a.unresolved_ambiguities?' · 미해결 '+a.unresolved_ambiguities+'건':''}<details><summary>초안 식별 정보</summary>${esc(a.record_id)}</details>`:`${esc(a.judge_model_name||a.judge_model_id)} · <span class="tag">${a.schema_version===2?'개체 분석 v2':'구형 언급 분석'}</span><details><summary>분석 식별 정보</summary>분석 ID: ${esc(a.analysis_id)}<br>실행: ${esc(a.analysis_run_id)}<br>Judge: ${esc(a.judge_version_id)}</details>`;
  currentEntities=entitiesOf(a);entityKey='';$('entity-search').value='';$('recommended').checked=false;$('category').value='all';$('sentiment').value='all';
  if(a)renderEntities();
}
function renderEntities(){
  const q=$('entity-search').value.trim().toLocaleLowerCase(),only=$('recommended').checked;
  const entities=currentEntities.filter(e=>(!only||e.is_recommended)&&`${e.brand||''} ${e.model||''}`.toLocaleLowerCase().includes(q));
  if(!entities.some(e=>e.key===entityKey))entityKey=entities[0]?.key||'';
  const legacy=currentAnalysis?.schema_version!==2;
  $('entity-summary').textContent=`${legacy?'언급':'개체'} ${entities.length}개 / 전체 ${currentEntities.length}개${legacy?' · 순위 정보 없음':' · 동률 순위 포함'}`;
  $('entities').innerHTML=entities.map(e=>`<tr class="${e.key===entityKey?'active':''}"><td><button class="entity-name" data-entity="${esc(e.key)}" aria-pressed="${e.key===entityKey}"><span>${esc(e.brand||'브랜드 미지정')}</span><strong>${esc(e.model||'브랜드 전체')}</strong></button></td><td>${e.is_recommended?'추천':'—'}</td><td>${e.rank??'—'}</td><td>${e.kbf_assessments.length}</td></tr>`).join('')||`<tr><td colspan="4" class="blank">${currentEntities.length?'검색 결과 없음':'분석 성공 · 추출된 항목 없음'}</td></tr>`;
...
function renderEntity(){
  ...
  $('entity-title').textContent=[e.brand,e.model].filter(Boolean).join(' · ')||'브랜드·모델 미지정';
  $('recommendation').innerHTML=(e.is_recommended?e.legacy?'해당 언급에서 추천':`추천${e.rank!=null?' · '+e.rank+'위 (동률 가능)':''}`:'추천 대상 아님')+(e.recommendation_evidence?'<br><button class="small" data-recommendation>추천 근거 보기</button>':'');
...
$('entity-search').oninput=$('recommended').onchange=renderEntities;
...
$('recommendation').onclick=e=>{if(e.target.closest('[data-recommendation]'))showEvidence(currentEntities.find(x=>x.key===entityKey).recommendation_evidence);};
